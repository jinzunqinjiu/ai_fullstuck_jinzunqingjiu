<template>
    <div class="flex flex-col h-screen">
        <div class="flex flex-nowrap fixed w-full items-baseline top-0 px-6 py-4 bg-gray-100">
            ...
            <div @click="clickConfig()" class="ml-auto px-3 py-2 text-sm cursor-pinter hover:bg-white rounded-md">
                设置
            </div>
        </div>
    ...
    <div class="sticky bottom-0 w-full p-6 pb-8 bg-gray-100">
        ...
        <div class="flex">
            <input class="input" :type="isConfig ? 'password' : 'text'" :placeholder="isConfig ? 'sk-xxxxxxxx' : '请输入'"
                v-model="messageContent" @keydown.enter="">
...
const saveAPIKey = (apiKey: string) => {
    ...
    // 加密后的APIKey
    const aesAPIKey = CryptoJS.AES.encrypt(apiKey, getSecretKry()).toString();
...
const sendOrSave = () => {
    ...
    if (isConfig.value) {
        ...
        clearChatMessageContent();
    } else {
        sendChatMessage();
    }
}

const clearChatMessageContent = () => {
...
const getAPIKey = () => {
    ...
    apiKey = CryptoJS.AES.decrypt(aesAPIKey, getSecretKry()).toString(CryptoJS.enc.Utf8);
    return apiKey;
}
